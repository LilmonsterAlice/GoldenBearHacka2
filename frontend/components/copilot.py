...
def _mock_idle_interactive_response(question: str) -> dict[str, Any]:
    ...
    if normalized.strip(" .!?") in {"hello", "hi", "hey", "你好", "嗨"}:
        answer = (
            "Hello! I can explain the Idle Interactive opportunity, the evidence "
            "behind it, or what could go wrong if an idle-timeout policy is used."
        )
        risk = "No recommendation has been evaluated yet."
        recommendation = "Choose a suggested question or ask about the GPU analysis."
        evidence = []
        confidence = None
        ...
        answer = (
            "I can explain GPU spending and the Idle Interactive opportunity, "
            "summarize supporting findings, describe policy risks, and distinguish "
            "observed evidence from recommendations."
        )
        risk = "No recommendation has been evaluated yet."
        recommendation = (
            "Ask about recoverable capacity, evidence, cost if wrong, confidence, "
            "or safe policy rollout."
        )
        evidence = []
        confidence = None
    elif not any(
        term in normalized
        for term in (
            "gpu", "idle", "interactive", "recover", "capacity", "utilization",
            "allocation", "timeout", "warning", "saving", "cost", "risk",
            "evidence", "finding", "job", "recommendation", "policy",
            "confidence", "caveat", "cluster", "compute", "research", "waste",
        )
    ):
        answer = (
            "The backend LLM is unavailable, so I cannot answer this general "
            "question right now."
        )
        risk = "Not applicable."
        recommendation = "Try again when the backend is available."
        evidence = []
        confidence = None
    elif "wrong" in normalized or "risk" in normalized:
        answer = (
...
def _render_assistant_response(response: dict[str, Any]) -> None:
    ...
    has_grounded_analysis = bool(
        response.get("evidence")
        or response.get("finding_ids")
        or response.get("job_ids")
        or response.get("confidence") is not None
    )
    confidence = response.get("confidence")
    if has_grounded_analysis and confidence is not None:
        st.caption(f"Confidence: {confidence:.0%}")

    caveats = response.get("caveats", [])
    if any("generated by Featherless" in caveat for caveat in caveats):
        if response.get("evidence") or response.get("finding_ids") or response.get("job_ids"):
            st.caption("Source: Featherless · Citations controlled by MantisGrid grounding")
        else:
            st.caption("Source: Featherless")
    elif any("deterministic grounded response" in caveat for caveat in caveats):
        st.caption("Source: deterministic grounded fallback")
    elif any("local UI fallback" in caveat for caveat in caveats):
        st.caption("Source: local UI fallback")

    if not has_grounded_analysis:
        return
...
def _render_copilot_window() -> None:
    ...
    _initialize_history()
    pending_question = st.session_state.get("copilot_pending_question")
    with st.container(key="copilot_window"):
        ...
        with st.container(height=300, border=False, key="copilot_history"):
        ...
        if question and not pending_question:
        ...
        try:
            ...
            response = _mock_idle_interactive_response(pending_question)
        ...
        del st.session_state.copilot_pending_question
        st.rerun()
...
def render_copilot() -> None:
    ...
    st.markdown(
        ...
        .st-key-copilot_composer [data-testid="stCaptionContainer"] p {
            font-size: 0.72rem;
            margin-bottom: 0.15rem;
        }
        .st-key-copilot_composer [data-testid="stVerticalBlock"] {
            gap: 0.28rem;
        }
        ...
        .st-key-copilot_composer [data-testid="stChatInput"] textarea {
